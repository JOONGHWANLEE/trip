...
import com.example.trip.util.MyUtil;
...
public class TripController {
	...
	//get방식으로 request가 들어올때
		...
		public String createdOK(Trip trip, HttpServletRequest request, Model model) {
			
			
			try {
				int maxNum = tripService.maxNum();
				
				trip.setNum(maxNum + 1); //num컬럼에 들어갈 값을 1증가시켜준다
				
				tripService.insertData(trip);
				
			} catch (Exception e) {
				e.printStackTrace();
				model.addAttribute("errorMessage", "게시글을 작성하는 중 에러가 발생했습니다.");
				return "bbs/created";
			}
		...
		public String list(Trip trip, HttpServletRequest request, Model model) {
			
			try {
				String pageNum = request.getParameter("pageNum"); //바뀌는 페이지 번호
				int currentPage = 1; //현재페이지 번호(디폴트는 1)
				...
				//3. 전체 게시물 리스트를 가져온다.
				List<Trip> lists = tripService.getLists(searchKey, searchValue, start, end);
				...
				model.addAttribute("pageIndexList", pageIndexList); //게시물 하단의 버튼 ◀이전 1 2 3 4 5 다음▶
		...
		public String article(HttpServletRequest request, Model model) {
			try {
				...
				//1. 조회수 늘리기
				tripService.updateHitCount(num);
				
				//2. 게시물 데이터 가져오기
				Trip trip = tripService.getReadData(num);
				...
				// 게시글의 라인수를 구한다.
				int lineSu = trip.getContent().split("\n").length;
		...
		public String updatedOK(Trip trip, HttpServletRequest request, Model model) {
			String pageNum = request.getParameter("pageNum");
			String searchKey = request.getParameter("searchKey");
			String searchValue = request.getParameter("searchValue");
			String param = "?pageNum=" + pageNum;
			...
			try {
				trip.setContent(trip.getContent().replaceAll("<br/>", "\r\n"));
				tripService.updateData(trip);
